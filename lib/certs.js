...
var createCA = function (configPath) {

    try {
        fs.statSync(configPath + '/ca-key.pem');
        fs.statSync(configPath + '/ca-cert.pem');

        console.log(('Certificate Authority key material already exists under ' + configPath).green);
        return;
    } catch (err) {
    ...
    fs.writeFileSync(configPath + '/ca-key.pem', keyPem);
    fs.writeFileSync(configPath + '/ca-cert.pem', certPem);
    ...
    console.log(('The CA certificate that you need to import is: ' + configPath + '/ca-cert.pem').yellow);
    ...
    if (os.type() === 'Darwin') {
        ...
        //console.log("\n    sudo security add-trusted-cert -d -r trustRoot -p basic " + configPath + "/ca-cert.pem")
    }
};

var loadCA = function (configPath) {
    var keyPem = fs.readFileSync(configPath + '/ca-key.pem', 'utf8');
    var certPem = fs.readFileSync(configPath + '/ca-cert.pem', 'utf8');
